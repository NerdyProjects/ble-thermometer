#ifndef _OTA_SERVICEMANAGER_CONFIG_H_
#define _OTA_SERVICEMANAGER_CONFIG_H_
...
/* Enable/disable Data length extension Max supported ATT_MTU size based on OTA client & server Max ATT_MTU sizes capabilities */
    ...
    #define OTA_MAX_ATT_MTU_SIZE    (220)              /* OTA Client & Server supported ATT_MTU */   
#else /* BlueNRG-1 device: no extended packet length support */
...
#define ATT_VALUE_ARRAY_SIZE    (44 + 99 + (4 + (OTA_16_BYTES_BLOCKS_NUMBER * 16))) 
...
#define MAX_ATT_SIZE            (4 + (OTA_16_BYTES_BLOCKS_NUMBER * 16))  /* max value which can be supported on DTM client side to fit a single ATT packet */
...
#endif // _OTA_SERVICEMANAGER_CONFIG_H_
